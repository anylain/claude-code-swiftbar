...
`cc-status-writer` 和 `cc-meta-writer` 是 Claude Code hook 脚本，被 `UserPromptSubmit` 等**阻塞型 hook** 调用 —— 任何非零退出会 block 用户输入框，整套 Claude Code 失灵直到脚本被修复或卸载。所以改这两个文件**必须**：

1. 改完先 `bash -n <path>` 静态语法检查（毫秒级，0 副作用）
2. 用真实 payload 跑一次：`echo '<json>' | bash <path>`，看 exit code 是 0
3. 脚本头部已有 `set -uo pipefail` + `trap 'exit 0' ERR` + 末尾 `exit 0` 三重兜底；**不要**改回 `set -e` 或删掉 trap，宁可状态文件偶尔脏也不能 block 用户
